...
async def on_new_private_message(event):
    ...
    if event.chat_id == 1050898456:
        await event.client.send_message(chat, "**⪼ اطـلق هـلاو مطورتي الغـالية ارمكس اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰*")
        reason = "**يجب عليك الترحيب بمطور السورس **"
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق ارمكس
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@sbb_b.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق ارمكس
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق ارمكس
...
async def approve_p_m(event):
    ...
    if len(approved_users) > 0:
        for user in approved_users:
            APPROVED_PMs += f"• 𝐀𝐑𝐌𝐗  𝐒𝐎𝐔𝐂𝐑𝐄 ََ𝗲 - 𝑨𝑷𝑷𝑹𝑶𝑽𝑬𝑫𝑺 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"
    else:
