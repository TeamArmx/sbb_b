...
LOGS = logging.getLogger("اعداد ارمكس")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@X_Error_404_X"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                ...
                buttons=[(Button.url("مطور السورس", "https://t.me/X_Error_404_X"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/armxsource.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت ارمكس", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
